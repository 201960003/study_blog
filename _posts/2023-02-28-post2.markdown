...
1 / 2 == 1.0 / 2.0; // 0.5 참
```
Javascript는 표준 산술 연산자(```+```,```-```,```*```,```/```)외에도 아래의 표에서 나열된 산술 연산자를 제공합니다.
...
<code>false</code> 로 변환할 수 있는 표현식은 평가 결과가 ```null```. ```0```, ```NaN```, 빈 문자열(```""```),```undefined```인 경우입니다.
...
```
<span style="color:grey">문자열은 true로 반환하는 것 같다..<span>
